...
;
; (require :reload-all '[bikeshed.core]) (ns bikeshed.core)
;
(defn foo
...
(defn bikeshed
  ...
  (let [options (first opts)
        ...
        long-lines (if (nil? (:max-line-length options)) (long-lines all-dirs)
                     (long-lines all-dirs :max-line-length (:max-line-length options)))
